...
#ifndef UNICODE
template<>
inline const TCHAR* ToString(const char*const object, TCHAR buffer[], size_t bufferSize)
{
    ::_sntprintf_s(buffer, bufferSize, _TRUNCATE, _T("%s"), object);
    return buffer;
}


template<>
inline const TCHAR* ToString(const std::string& object, TCHAR buffer[], size_t bufferSize)
{
    return ToString(object.c_str(), buffer, bufferSize);
}
